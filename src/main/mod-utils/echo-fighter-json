...
import * as readline from 'readline'; // Note: readline is imported but unused in the original script
...
// 2. Declare global variables to support original globalThis usage
declare global {
  var vanillaData: VanillaData;
  var fighterData: VanillaData;
  var resultingConfig: ResultingConfig;
...
function makeDirsFromFile(filePath: string): void {
...
function findFighterFiles(filesDirectory: string): string[] {
  const allFiles: string[] = [];
  const stack: string[] = [filesDirectory]; // Start with the root directory
...
export function init(
  ...
  globalThis.knownFiles = new Set(
    fs.readFileSync(hashesFile, 'utf-8').split('\n').map((x) => x.trim())
  );
...
export function reslotFighterFiles(
  ...
  outDir: string,
  fighterName: string,
  echoColorStart: number,
  numColorSlots: number,
  baseEchoSlot: string
): void {
  const newDirInfos: string[] = [];
  const newDirInfosBase: Record<string, string> = {};
  const shareToVanilla: Record<string, string[]> = {};
  const shareToAdded: Record<string, string[]> = {};
  const newDirFiles: Record<string, string[]> = {};
  ...
  const combinedShares: Record<string, string[]> = { ...shareToVanilla, ...shareToAdded };
  ...
  const sortedNewDirFiles: Record<string, string[]> = {};
  ...
  // Step 5: Sort `share-to-vanilla` and `share-to-added`
  const sortedShareToVanilla: Record<string, string[]> = {};
  ...
  const sortedShareToAdded: Record<string, string[]> = {};
